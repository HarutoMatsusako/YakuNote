...
export default function SummariesPage() {
  ...
  const fetchSummaries = async (skip = 0, limit = 10) => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/summaries/${user.id}?skip=${skip}&limit=${limit}`);
          ...
          <>
                    ...
                    <tr key={summary.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-normal">
                        <div className="text-sm text-gray-500">
                          {summary.url ? (
                            <a href={summary.url} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">
            ...
            {pagination.total > pagination.limit && (
              <div className="flex justify-between items-center mt-6">
                <button
                  onClick={() => changePage(Math.max(0, pagination.skip - pagination.limit))}
                ...
                <div className="text-sm text-gray-500">
                  {pagination.skip + 1} - {Math.min(pagination.skip + pagination.limit, pagination.total)} / {pagination.total} 件
                </div>
